...
const ImageGrid = ({ setSelectedImage }) => {
  ...
  const dltHendler = (e, name, id) => {
    ...
    imgref.delete().then(() => {
      ...
      console.log(`oops! error deleting image`);
    });
  ...
  return (
    <div className="img-grid">
      {docs && docs.map(doc => {
        return (<motion.div className="img-wrap" key={doc.id} onClick={(e) => clickHandler(e, doc.imageUrl)} layout>
          <motion.img src={doc.imageUrl} alt="uploaded img" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .5 }} />
